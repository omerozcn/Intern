...
namespace TicketSystem.Controllers
{
     ...
     public class AppUserTicketController : ControllerBase
     {
          ...
          [HttpPost]
          public async Task<IActionResult> AddUserTickets(string title)
          {
               var username = User.GetUserName();
               var appUser = await _userManager.FindByNameAsync(username);
               var ticket = await _ticketRepo.GetByNameAsync(title);

               if (ticket == null) return BadRequest("Ticket not found");

               var userTicket = await _userticketRepo.GetUserAppUserTicket(appUser);

               if (userTicket.Any(e => e.Title.ToLower() == title.ToLower())) return BadRequest("Cannot add same ticket to appuserticket");
